...
d_pop <- dir('.', pattern = 'DIS_FXE', full.names = TRUE) %>% 
  map_df(ler)
```
...
1. Não ficou claro o que significa alcançar 60% da taxa de atendimento de crianças de zero a três anos.
    1. Sabe-se que a população muda ao longo do tempo. Logo, faz sentido utilizar a projeção da população dada pela Fundação SEADE.
    ...
    1. O número obtido (terceira linha da tabela) para 2016 não é coerente com o número de vagas de 85266 a ser criado.
...
d_pop %>% 
  ...
  mutate(`Projeções` = c('População', 
                 'Meta (População * 60%)', 
                 'Diferença entre meta e matriculados')) %>% 
  knitr::kable()
```
...
d_tidy %>% 
  filter(ano == 2016) %>% 
  select(-demanda, -ano) %>% 
  mutate(meta = ceiling(pop * 0.6),
         diferenca = meta - matricula) %>% 
  filter(diferenca > 0) %>% 
  arrange(desc(diferenca)) %>% 
  setNames(c('Distrito', 'Matriculados', 'População', 'Meta', 'Diferença')) %>% 
